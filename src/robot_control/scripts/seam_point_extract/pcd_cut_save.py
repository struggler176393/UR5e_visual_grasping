...
def vis_cut_save(folder_path, point_cloud):
    ...
    while True:
        ...
        pcd = vis.get_cropped_geometry()   #open3d==0.17.0
        # points = vis.get_picked_points()             #open3d==0.13.0
        print(len(np.array(pcd.points)))

        if len(np.array(pcd.points)) == len(np.array(point_cloud.points)):        #open3d==0.17.0
        # if len(np.array(points)) == 0:        #open3d==0.13.0
